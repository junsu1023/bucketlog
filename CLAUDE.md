...
| 단계 | **Phase 0 (MVP) 진행 중** — 4주차까지 완료(1~4주차 상세는 이전 기록 참고) + 6주차 일부: 다크모드
  ...
  네이티브 렌더링) + 전체 데이터 초기화(M-03) + 미구현 P0 스펙 3개(G-06 반복형 완료 제안, E-04
  기록 날짜 수정, E-05 기록 수정/삭제) 발견 후 구현. 이후 사용자가 제공한 목업을 바탕으로
  "Archive Your Moments" 디자인 시스템 전면 개편 — 컬러/타이포(Playfair Display·JetBrains Mono
  임베드) + 하단 내비게이션(홈/보관함/검색/설정) 도입 + 검색 기능 신규 + 홈/목표상세(기록·정보
  탭)/보관함/설정/온보딩 화면 레이아웃 전부 재설계. 자세한 내용은 `docs/DESIGN.md`, `test.md`
  §12 참고. 알림(5주차)도 스마트 넛지·월간 회고·목표별 리마인더·알림 설정·백업복원까지 먼저
  구현됨. 이후 남아있던 P1 스펙 5개(G-09 마감일, G-11 정렬·필터, G-12 연말이월, N-04 마감임박,
  ...
  앞당겨 구현** — 처음엔 타입별 위젯 3종(오늘의 기억/지금 어때요/마감 임박, Jetpack Glance)으로
  만들었으나, 이후 사용자가 제공한 목업(마스코트 "루미" 포함)을 바탕으로 **크기별 위젯 3종으로
  전면 재설계**: Small "오늘의 한 걸음"(추천 목표 1개 + CTA), Medium "2026년의 나"(연도 진행률
  링 + 다음 목표 카드), Large "오늘의 추억"(H-07 작년 오늘 사진+메모). Todo 위젯처럼 보이지
  않게 하는 게 핵심 기준 — 여러 목표를 리스트로 보여주지 않음(스트릭/체크리스트처럼 보일 위험,
