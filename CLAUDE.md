...
| 항목 | 값 |
|---|---|
| 단계 | **Phase 0 (MVP) 진행 중** — 4주차까지 완료: Room DB, repository/usecase, 홈+목표생성+퀵체크인(1주차) / 사진 파이프라인 + 진행 기록(E-02) + 완료 인증 사진 첨부(2주차) / 목표 상세 세로 타임라인(D-01~D-04) + 완료·접어두기·되돌리기(G-06/07/08) + 사진 전체화면 뷰어(D-05) + 목표 수정(G-05) + 회고 프롬프트(E-08)(3주차) / 홈 H-01~H-07("작년 오늘" 배너 포함) + 보관함 A-01~A-04(전체 타임라인·간단 통계 포함) + 온보딩 O-01~O-03(4주차). 알림(5주차 항목)도 스마트 넛지·월간 회고·목표별 리마인더·알림 설정·백업복원까지 먼저 구현됨. 전부 Android 실기기/에뮬레이터 검증함(`test.md` 참고) |
| 앱 이름 | **가칭** BucketLog / 버킷로그 |
| 백엔드 | **없음** (MVP는 로컬 전용. Phase 1에 도입) |
| 로그인 | **없음** (Phase 1) |
| 다음 작업 | `docs/MVP-SCOPE.md` §4의 6주차 — 완료 카드 공유 · 다크모드 마감 · 버그 · 스토어 제출 준비. 5주차 알림 중 남은 N-04(마감임박, G-09 마감일 UI 선행)·N-05(연말회고, G-12 연말이월 선행)는 각 선행 기능과 묶어 이후 진행 |
