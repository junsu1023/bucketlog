...
| 항목 | 값 |
|---|---|
| 단계 | **Phase 0 (MVP) 진행 중** — 2주차까지 완료: Room DB, repository/usecase, 홈+목표생성+퀵체크인(1주차) / 사진 파이프라인(ImageProcessor·FileStorage·CameraCapture·PhotoPicker) + 진행 기록(E-02) + 완료 인증 사진 첨부(2주차). 갤러리 선택→압축→저장→홈 카드 대표 사진 렌더링까지 Android 실기기 검증함 |
| 앱 이름 | **가칭** BucketLog / 버킷로그 |
| 백엔드 | **없음** (MVP는 로컬 전용. Phase 1에 도입) |
| 로그인 | **없음** (Phase 1) |
| 다음 작업 | `docs/MVP-SCOPE.md` §4의 3주차 — 목표 상세 타임라인 UI(사진 포함), 완료/접어두기 플로우. **제품이 처음 "제품처럼" 보이는 시점** |
